 ...
 * you may not use this file except in compliance c_with the License.
...
c_match : ( OPTIONAL sp )? MATCH ws pattern ( ws where )? ;

unwind : UNWIND ws expression sp AS sp variable ;

c_merge : MERGE ws patternPart ( sp mergeAction )* ;

mergeAction : ( ON sp MATCH sp c_set )
            | ( ON sp CREATE sp c_set )
            ;

create : CREATE ws pattern ;

c_set : SET setItem ( ',' setItem )* ;
...
c_delete : ( DELETE expression ( ',' expression )* )
...
c_with : ( WITH DISTINCT sp returnBody where? )
     | ( WITH sp returnBody where? )
     ;

c_return : ( RETURN sp DISTINCT sp returnBody )
 ...
 * And extended c_with a few characters.
 */
...
/**
 * Any character except "`", enclosed within `backticks`. Backticks are escaped c_with double backticks. */
